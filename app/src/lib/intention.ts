/**
 * Intention data structures and URL matching logic for the Intender mobile app.
 *
...
export function createIntentionIndex(intentions: Intention[]): IntentionIndex {
  ...
  for (const [domain, intentionScopes] of index) {
    intentionScopes.sort(
      (a: { scope: IntentionScope }, b: { scope: IntentionScope }) =>
        b.scope.urlLength - a.scope.urlLength
    );
  }
